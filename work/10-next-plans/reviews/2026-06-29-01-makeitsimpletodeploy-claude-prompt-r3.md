# Claude re-review prompt: make JuriSearch simple to deploy, r3
...
- Confirm whether the r2 findings about explicit `site provision-db`, embedder fingerprint derivation,
  normalize/pooling, and admin/bootstrap DB identity are resolved.
...
Validation already run after the r3 update:

- ASCII scan passed: `LC_ALL=C rg -n '[^ -~]' work/10-next-plans/01-makeitsimpletodeploy.md` returned no
  matches.
- Stale-wording scan for the prior issue phrases returned no matches.
