...
- **Combined recipe to prove in 0.3:** (1) bring up `postgresql_embedded` (gciauto2 pattern), pinning the PG major to one the fork supports; (2) build the fork's `pg_search` and `pgvector` for that **same major** via `cargo pgrx package`; (3) copy the artifacts into the embedded install's `pkglibdir`/`sharedir`; (4) add `pg_search` to `shared_preload_libraries` in the conf map, then `CREATE EXTENSION pg_search; CREATE EXTENSION vector;`.
...
| 0.3 Embedded Postgres spike | `cargo-pgrx 0.18.1` + PG server headers; reuse `gciauto2` lifecycle + `paradedb` fork; pick PG major (17/18); ABI-match the `pg_search`/`pgvector` build and drop into the embedded data dir. |
