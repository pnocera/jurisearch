...
- `jurisearch-storage` starts `initdb`/`pg_ctl` from the selected `pg_config --bindir`, writes local-only runtime config, and runs durable lifecycle, migration, BM25, and vector smokes against that child process.
...
```bash
JURISEARCH_REQUIRE_PG_EXTENSIONS=1 JURISEARCH_PG_CONFIG=/path/to/pg_config cargo test -p jurisearch-storage
```
...
- A managed-child-Postgres lifecycle failure triggers the no-Postgres fallback path only after the lifecycle failure is recorded with logs and reproduction steps.
