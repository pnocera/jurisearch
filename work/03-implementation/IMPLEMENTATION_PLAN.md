...
- Remaining: promote release-gating fixtures only after named human legal-domain review, record reranker adoption/deferral benchmark evidence, and decide whether the current release-gating fixture set is strong enough to prove hybrid over BM25 (the current four fixtures do not, because BM25 and hybrid both pass).
- Next optimization: make `status` cheap by default. Current replay snapshots are correct but slow on the full corpus because they recompute ordered hashes over the full documents/chunks/embeddings/manifests tables and about 12.9M publisher edges on every call. Next implementation slice should split normal readiness from deep replay verification, materialize replay signatures after ingest/projection/dense finalization, expose explicit `status --deep` / refresh semantics for on-demand recomputation, and have default `status` read cached manifest-backed signatures.
