...
- Done: `jurisearch ingest legi-archives` streams the planned LEGI baseline/deltas into canonical storage for the current ARTICLE parser slice, records ingest run/member/error accounting, applies compatible resume decisions, counts unsupported roots, records safe-mode as run metadata, and optionally quarantines failed XML payloads by run/archive/member path. CLI contract tests cover a fresh-index ingest, failed-member quarantine, accounting rows, and a compatible replay skip.
