...
- Remaining (France-LEGI gate — the runner, design fixed): an `eval france-legi` subcommand that `open_index` once and, via `search_with_postgres`, runs the three official-evidence categories, computes per-category metrics, and emits the provenance-complete `phase1_france_legi_benchmark` JSON. **Gold from the index** (no archive re-parse): known-item = `documents` rows (`citation` query, gold `document_id`, `as_of=valid_from`); temporal = `documents.version_group` families with `valid_from`/`valid_to` windows; cross-reference = `graph_edges` — but **verified-in-source caveat**: publisher edges store a uniform `relation="refers_to"` and **`to_document_id` is NULL** (graph materialization unresolved), so cross-reference gold must read `payload->>'to_source_uid'` filtered to `typelien="CITATION"` from the edge `payload` jsonb and join to `documents.source_uid` (cannot use resolved edges). Then run one calibration sweep to confirm/raise floors and settle the known-item as-of policy. This is a multi-phase build: (F) a storage `france_legi_gold` qrel-extraction helper (tested vs synthetic managed PG), (G) the `eval france-legi` subcommand (loop + metrics + artifact emission).
