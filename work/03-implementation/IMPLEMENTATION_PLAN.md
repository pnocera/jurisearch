...
- Operator note: indexes populated before the `canonical_record:v2` compatibility bump should be reprocessed intentionally; a blind resume will surface pre-bump members as compatibility mismatches before metadata backfill occurs.
...
- Operator note: until an explicit full hierarchy rebuild command or per-run backfill completion marker exists, an interrupted ingest after member persistence but before end-of-run scoped backfill should be recovered with an intentional full hierarchy rebuild.
...
- Done: ignored real-data coverage now scans the local official LEGI baseline (`/home/pierre/Apps/juridocs/opendata/LEGI/Freemium_legi_global_20250713-140000.tar.gz`) until it observes status and temporal-end evidence. The verified run on 2026-06-21 visited 10,322 XML members, parsed 8,175 ARTICLE members, observed `VIGUEUR`, `MODIFIE`, `ABROGE`, `ANNULE`, `MODIFIE_MORT_NE`, `PERIME`, and `TRANSFERE`, found finite `valid_to` examples for the non-open-ended statuses, and confirmed the `2999-01-01` sentinel normalization. It records, rather than fails on, current parser gaps in the early `TNC_non_vigueur` archive branch such as missing `BLOC_TEXTUEL/CONTENU`, `META_ARTICLE/TYPE`, and `META_ARTICLE/NUM`, and guards that this real-data parse-error rate stays below 1%. Multi-version and same-day-version corpus evidence remains future work because current canonical `version_group` is still the per-version `LEGIARTI` id rather than a shared article chronicle key.
- Remaining for later 1.1 slices: use persisted `TEXTELR` metadata for broader text-structure and multi-link hierarchy assembly across article versions, close full-corpus ARTICLE parser gaps for official files that omit currently-required fields, and add maintenance batching or a CLI command for explicit full hierarchy rebuilds if the full corpus requires it.
