...
- Remaining for later 1.1 slices: persist text/section metadata in canonical storage, use `TEXTELR`/`SECTION_TA` to assemble full hierarchy paths across article versions, expand status/temporal fixtures across the full LEGI corpus, and publish coverage/freshness manifest fields.
