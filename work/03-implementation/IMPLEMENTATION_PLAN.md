...
- Remaining (France-LEGI gate): (1) build the artifact **runner** that executes the production pipeline over the three official-evidence categories and emits the `phase1_france_legi_benchmark` JSON (with the structured provenance now required); (2) run one full production-pipeline **calibration** sweep to confirm/raise floors, settle the known-item as-of policy, and pull temporal qrels from `documents.version_group`/`valid_from`/`valid_to`. Scaling note (verified in source): `search_payload` opens its own embedded PG via `open_index`→`ManagedPostgres::start_durable` and drops it on return (`crates/jurisearch-cli/src/main.rs:770`,`:2763`), so PG is **cold-started per query** for both `session` and `eval phase1`. The curated `eval phase1` set is fast (few fixtures); a large automatic qrel sweep (the 100-query `external-benchmarks/legi_live_proto.py` route) is infeasible until a persistent-PG batch-eval path (one PG lifecycle, N queries) is added — that earlier timed out on repeated cold starts, not on feasibility.
