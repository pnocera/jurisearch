...
- Done: `TEXTELR` canonical metadata now preserves ordered `structure_links` for `LIEN_TXT`, `LIEN_SECTION_TA`, and `LIEN_ART`, including target UID, `debut`/`fin`, optional `niv`, link text, and raw DILA attributes. This ports the reusable `juridocs` text-structure insight into the persisted metadata layer without adding a new table yet.
- Remaining for later 1.1 slices: use persisted `TEXTELR.structure_links` plus `SECTION_TA` metadata for broader text-structure and multi-link hierarchy assembly across article versions, and add maintenance batching for full hierarchy rebuilds if corpus-scale runs require it.
