...
- Remaining for later 1.1 slices: use persisted `TEXTELR` metadata for broader text-structure and multi-link hierarchy assembly across article versions, decide how to represent or skip official ARTICLE records with no textual body, and add maintenance batching or a CLI command for explicit full hierarchy rebuilds if the full corpus requires it.
