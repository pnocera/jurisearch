...
- Remaining for later 1.1 slices: use persisted `TEXTELR` metadata for broader text-structure and multi-link hierarchy assembly across article versions, scope or batch hierarchy backfill for full-corpus incremental runs, and expand status/temporal fixtures against the full official LEGI corpus.
