...
- Remaining (France-LEGI gate, to OPEN the claim): close the retrieval-quality gap surfaced by the calibration — query construction (lead with the discriminating article number), a cross-encoder reranker over the fused top-N, and/or intent routing (BM25-dominant for high-lexical-overlap exact-citation queries); plus a cross-reference gate-task redesign (citation-span queries) or an explicit floor re-scoping. Then re-run the sweep and wire `JURISEARCH_PHASE1_FRANCE_LEGI_BENCHMARK` to the passing artifact.
