...
- Done: `jurisearch ingest legi-archives` streams the planned LEGI baseline/deltas into canonical storage for the current ARTICLE parser slice, records ingest run/member/error accounting, applies compatible resume decisions, counts unsupported roots, records safe-mode as run metadata, and optionally quarantines failed XML payloads by run/archive/member path. Member-level failures are reported as `run_status: "failed"` with exit `0` when the command itself completed; systemic archive/storage errors still use JSON errors and non-zero exits. Unsupported roots are currently persisted as skipped member rows with the root name in `source_entity` plus the JSON `unsupported_roots` summary. CLI contract tests cover a fresh-index ingest, failed-member quarantine, accounting rows, compatible replay skip, failed-member retry, and compatibility-blocked replay.
- Done: retrieval commands now enforce ingest-health coverage outside `status`: `search` requires projection and embedding coverage before contacting the embedding endpoint, while `fetch` requires projection coverage for requested stored documents.
- Remaining for later 1.0 slices: compute replay snapshot status beyond the current `pending` storage placeholder, add safe-mode behavior for future optimized write/backfill paths, and expand full-corpus gate thresholds once broader LEGI canonicalization lands.
