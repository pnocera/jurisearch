...
- Remaining: run the full BSARD benchmark and record the artifact before setting `external_expert_annotated_eval` to pass. The artifact must record dataset revision, jurisdiction, eval-only usage scope, license implications, thresholds, and the Belgian-law to French-LEGI applicability argument, and it must clear the status-enforced hybrid floors (`recall_at_20 >= 0.75`, `ndcg_at_20 >= 0.60`, `mrr_at_20 >= 0.50`).
