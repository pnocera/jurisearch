...
- Remaining for later 1.1 slices: use persisted `TEXTELR` metadata for broader text-structure and multi-link hierarchy assembly across article versions, add maintenance batching for explicit full hierarchy rebuilds if the full corpus requires it, and expand status/temporal fixtures against the full official LEGI corpus.
