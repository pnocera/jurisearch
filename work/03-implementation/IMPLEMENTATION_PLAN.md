...
- Remaining for later 1.0 slices: compute replay snapshot status beyond the current `pending` storage placeholder, add safe-mode behavior for future optimized write/backfill paths, block/mark query access when required projection gates fail outside the status report, and expand full-corpus gate thresholds once broader LEGI canonicalization lands.
