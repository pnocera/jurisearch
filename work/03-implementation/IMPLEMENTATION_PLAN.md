...
- Remaining (France-LEGI gate): run the **calibration sweep** (`jurisearch eval france-legi --out <artifact>` over the production index, single PG lifecycle) and wire `JURISEARCH_PHASE1_FRANCE_LEGI_BENCHMARK` to the artifact → flips `france_legi_official_eval` and (if floors are met) opens the Phase 1 claim; confirm/raise floors and settle the known-item as-of policy from the observed numbers.
