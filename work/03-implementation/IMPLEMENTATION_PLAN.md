...
- Done: `jurisearch ingest backfill-legi-hierarchy` runs the explicit full LEGI hierarchy rebuild over an existing index, reports updated-document and invalidated-embedding counters, and leaves ingest-run/member accounting untouched. This is the operator recovery path for interrupted runs after member persistence but before end-of-run scoped backfill.
