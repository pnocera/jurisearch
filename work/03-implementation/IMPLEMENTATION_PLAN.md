...
- Remaining for later 1.0 slices: compute replay snapshot status beyond the current `pending` storage placeholder, add safe-mode behavior for future optimized write/backfill paths, expand full-corpus gate thresholds once broader LEGI canonicalization lands, and make embedding coverage fingerprint-aware before Phase 1.7 re-embedding/migration gates.
