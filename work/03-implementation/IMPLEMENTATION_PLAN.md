...
- Remaining for later 1.1 slices: assemble deeper multi-level TEXTELR section ancestry directly from ordered `niv` transitions when `SECTION_TA.hierarchy_path` is incomplete, capture `EXPLAIN (ANALYZE)` evidence on a real loaded code before corpus-scale TEXTELR backfills, and add maintenance batching for full hierarchy rebuilds if corpus-scale runs require it.
