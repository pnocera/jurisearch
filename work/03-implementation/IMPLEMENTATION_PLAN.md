...
- Remaining for later 1.1 slices: use persisted `TEXTELR` metadata for broader text-structure assembly across article versions, add temporal section-version selection instead of latest-section-wins enrichment, scope or batch hierarchy backfill for full-corpus incremental runs, expand status/temporal fixtures across the full LEGI corpus, and publish coverage/freshness manifest fields.
