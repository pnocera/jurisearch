...
- Done: focused article parser fixtures now pin LEGI status and temporal normalization for `VIGUEUR`, `MODIFIE`, `ABROGE`, `ABROGE_DIFF`, non-sentinel end dates, and both known LEGI open-ended sentinels (`2999-01-01` and `2999-12-31`).
- Remaining for later 1.1 slices: use persisted `TEXTELR` metadata for broader text-structure assembly across article versions, add temporal section-version selection instead of latest-section-wins enrichment, scope or batch hierarchy backfill for full-corpus incremental runs, and expand status/temporal fixtures against the full official LEGI corpus.
