...
- Remaining for later 1.2 slices: long-article structural sub-splitting, tokenizer-backed embedding preflight, corpus-scale indexed/materialized sibling lookup for interactive `context --siblings`, hierarchy-sensitive eval cases, and a search-ranking decision on whether BM25 should index `contextualized_body` instead of raw `body`.
