...
/// Dense candidates are post-filtered by zone, so over-fetch a wider ANN pool before ranking.
...
/// requested mode, scoped to `zone`. Mirrors `retrieval::ranked_candidate_ctes` over the zone tables.
fn ranked_zone_ctes(
...
lexical AS (
    ...
    FROM zone_units u
    WHERE u.search_body @@@ {query_text} AND u.zone = {zone}
...
dense_pool AS (
    ...
    FROM (
        ...
        WHERE e.embedding_fingerprint = {fingerprint}
    ) scored
...
dense AS (
    ...
    FROM dense_pool dp
    JOIN zone_units u ON u.zone_unit_id = dp.zone_unit_id
    WHERE u.zone = {zone}
...
pub fn zone_candidates_json(
    ...
    // Decision-metadata filters reuse the main predicate (each clause prefixed with ` AND`, against
    // `documents d`). Zone units are all decisions, so the predicate's `d.kind = 'decision'` is a no-op.
    let filter_predicate = query.decision_filters.predicate();
    let ranked_ctes = ranked_zone_ctes(query, &query_text, &zone_literal)?;
...
scored AS (
    ...
    WHERE u.zone = {zone_literal}{filter_predicate}
),
best_document AS (
