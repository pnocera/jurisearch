...
//! storage [`CitationLookup`]. Moved into `jurisearch-query` (work/09 P4-4B) so the site `cite`
//! handler can build its response without calling a CLI adapter; the CLI re-exports it unchanged.

use jurisearch_storage::citation::CitationLookup;

#[derive(Debug)]
pub enum ParsedCitationTarget {
    DocumentId {
        document_id: String,
...
impl ParsedCitationTarget {
    pub fn lookup(&self) -> Option<CitationLookup<'_>> {
        match self {
            Self::DocumentId {
                document_id,
                source_uid,
...
pub fn parse_citation_target(input: &str) -> ParsedCitationTarget {
...
pub fn extract_known_source_uid(value: &str, prefix: &str) -> Option<String> {
...
pub fn parse_article_number(normalized: &str) -> Option<String> {
...
pub fn article_number_token(candidate: &str) -> Option<String> {
    (candidate
        .chars()
...
pub fn detect_code_hint(normalized: &str) -> Option<String> {
...
pub fn contains_normalized_phrase(normalized: &str, phrase: &str) -> bool {
    let normalized = format!(" {normalized} ");
    let phrase = format!(" {phrase} ");
    normalized.contains(&phrase)
}

pub fn looks_like_nor(value: &str) -> bool {
...
pub fn normalize_citation_text(value: &str) -> String {
...
pub fn parse_pourvoi(input: &str) -> Option<String> {
